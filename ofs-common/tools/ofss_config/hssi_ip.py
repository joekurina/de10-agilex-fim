...
class HSSI(OFS):
    ...
    def __init__(self, ofs_config, hssi_config, target):
        ...
        # WORKAROUND for DE10-Agilex R24B package:
        # Quartus 24.1 has empty device_highspeed_tile info for R24B, causing
        # clock region constraint errors. R24A and R24B use the same AGFB014 die
        # with E-Tile in the same location. Force R24A during HSSI IP generation
        # to get proper E-Tile location info.
        if "R24B" in self.part:
            self.hssi_part = self.part.replace("R24B", "R24A")
            logging.info(f"HSSI Workaround: Using {self.hssi_part} for IP generation (actual target: {self.part})")
        else:
            self.hssi_part = self.part
    ...
    def set_deploy_cmd_args(self):
        """
        Override to use hssi_part (R24A) instead of part (R24B) for HSSI IP generation.
        This works because R24A and R24B use the same die with E-Tile in the same location.
        """
        deploy_args = ["ip-deploy"]
        deploy_args.append(f"--family={self.fpga_family}")
        # Use hssi_part (R24A) to get proper device_highspeed_tile info
        deploy_args.append(f'--part="{self.hssi_part}"')
